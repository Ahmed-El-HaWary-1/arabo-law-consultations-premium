...
import { Users, FileText, Phone, Calendar, Download, Search, Filter, LogOut } from 'lucide-react';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ bookings, language, isDark, onLogout }) => {
  ...
  const filteredBookings = bookings.filter(booking => {
    ...
    const matchesFilter = filterService === 'all' || booking.service?.title?.en?.includes(filterService);
    return matchesSearch && matchesFilter;
  });

  const stats = {
    totalBookings: bookings.length,
    todayBookings: bookings.filter(b => new Date(b.timestamp).toDateString() === new Date().toDateString()).length,
    pendingBookings: bookings.filter(b => !b.contacted).length,
    completedBookings: bookings.filter(b => b.contacted).length,
  };
                    ...
                    <td className="px-6 py-4">
                      {booking.files?.length > 0 ? (
                        <span className={`text-sm ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                          {booking.files.length} {language === 'ar' ? 'ملف' : 'files'}
                        </span>
                      ) : (
                        <span className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                          {language === 'ar' ? 'لا توجد ملفات' : 'No files'}
                        </span>
                      )}
                    </td>
                    <td className="px-6 py-4">
